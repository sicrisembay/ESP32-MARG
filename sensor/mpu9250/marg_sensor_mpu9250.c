...
static dataXYZf_t magSensitivityAdj = {0.0f};
static dataXYZf_t magBias = {0.0};
...
static esp_err_t _mpu9250_calGyroAccel(dataXYZf_t *pGyroBias, dataXYZf_t *pAccelBias)
{
    ...
    const uint16_t  gyrosensitivity  = 131;    /* 32768 / 250dps */
    const uint16_t  accelsensitivity = 16384;  /* 32768 / 2g */
    ...
    vTaskDelay(15);
    _i2c_write_byte(MPU9250_DEV_ADDR, ADDR_CONFIG, 0x02);       // Set low-pass filter to 92 Hz
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        _i2c_read(MPU9250_DEV_ADDR, ADDR_FIFO_R_W, 12, &data[0]);
        /* Note: Data pushed into FIFO is not consistent with datasheet
         *   ACCEL_Z --> ACCEL_Y --> ACCEL_X -->
         *   GYRO_Z --> GYRO_Y --> GYRO_X
         */
        accel_temp[2] = (int16_t) (((int16_t)data[0] << 8) | data[1]  ) ;
        accel_temp[1] = (int16_t) (((int16_t)data[2] << 8) | data[3]  ) ;
        accel_temp[0] = (int16_t) (((int16_t)data[4] << 8) | data[5]  ) ;
        gyro_temp[2]  = (int16_t) (((int16_t)data[6] << 8) | data[7]  ) ;
        gyro_temp[1]  = (int16_t) (((int16_t)data[8] << 8) | data[9]  ) ;
        gyro_temp[0]  = (int16_t) (((int16_t)data[10] << 8) | data[11]) ;
...
#if(MPU9250_DEBUG_INIT == 1)
        ESP_LOGI(TAG, "Sample %02d: Accel (%d, %d, %d) Gyro (%d, %d, %d)", ii,
    ...
    if(pAccelBias->z > 0.0f) {
        pAccelBias->z = pAccelBias->z - 1.0f;
...
#if(MPU9250_DEBUG_INIT == 1)
    ESP_LOGI(TAG, "Bias Accel (%f, %f, %f) Gyro (%f, %f, %f)",
            pAccelBias->x, pAccelBias->y, pAccelBias->z,
            pGyroBias->x, pGyroBias->y, pGyroBias->z);
#endif /* #if(MPU9250_DEBUG_INIT == 1) */

    /* Construct the gyro biases for push to the hardware gyro bias registers, which are reset to zero upon device startup */
    /* NOTE: Another inconsistent behavior observed
     * Format of gyro input bias is 2000dps (datasheet specified 1000dps)
     * Thus, divide by 8 to convert from 250dps to 2000dps.
     */
    data[0] = ((-gyro_bias[0]/8)  >> 8) & 0xFF;
    data[1] = (-gyro_bias[0]/8)       & 0xFF;
    data[2] = ((-gyro_bias[1]/8)  >> 8) & 0xFF;
    data[3] = (-gyro_bias[1]/8)       & 0xFF;
    data[4] = ((-gyro_bias[2]/8)  >> 8) & 0xFF;
    data[5] = (-gyro_bias[2]/8)       & 0xFF;
...
#if(MPU9250_DEBUG_INIT == 1)
    ESP_LOGI(TAG, "Accel Offset Reg OTP (%d, %d, %d)",
            accel_bias_reg[0], accel_bias_reg[1], accel_bias_reg[2]);
#endif /* #if(MPU9250_DEBUG_INIT == 1) */

    // Construct total accelerometer bias, including calculated average accelerometer bias from above
    /* Subtract calculated averaged accelerometer bias
     * NOTE1: Another inconsistent behavior observed.
     * Input accel bias expects 8G (app note specifies 16G)
     * Thus, divide by 4 to convert from 2G to 8G.
     * NOTE2: Preserve bit0 of bias register (used for temperature compensation)
     */
    accel_bias_reg[0] -= ((accel_bias[0]/4) & ~0x01);
    accel_bias_reg[1] -= ((accel_bias[1]/4) & ~0x01);
    accel_bias_reg[2] -= ((accel_bias[2]/4) & ~0x01);
...
static void _mpu9250_task(void *pArg)
{
    ...
    /* Init AK8963 Device */
    if(ESP_OK != _ak8963_init(&magSensitivityAdj)) {
    ...
    while(1) {
        ...
        if(intStatusFlag & MPU9250_NOTIFY_INT_FLAG) {
             ...
             * mag : milli gauss
             */
            accelData.x = (float)(accelRawData.x) * accelScale;
            accelData.y = (float)(accelRawData.y) * accelScale;
            accelData.z = (float)(accelRawData.z) * accelScale;
            ...
            magData.x = ((float)(magRawData.x) * magScale * magSensitivityAdj.x) - magBias.x;
            magData.y = ((float)(magRawData.y) * magScale * magSensitivityAdj.y) - magBias.y;
            magData.z = ((float)(magRawData.z) * magScale * magSensitivityAdj.z) - magBias.z;
